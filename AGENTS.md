...
## Product and context
- Product requirements: `docs/game-design.md`; design inputs: `docs/architecture.md`, `docs/data-contracts.md`, `docs/mvp-roadmap.md` and approved ADRs. Read task-relevant sections. **For BMAD discovery and requests for a brain dump, these files are already-supplied owner input; do not ask the owner to repeat them.** Identify only genuine unanswered decisions with precise file references, while following explicit official BMAD response/approval gates. Resume an existing BMAD draft at its recorded step; do not restart it merely because a prerequisite was missing.
- These documents describe intended behavior and decisions, **not whether a feature has been implemented or a BMAD artifact approved**. Existing technical documents do not automatically count as a finalized official BMAD PRD, architecture or UX contract.
